...
struct Scheduler {
    ...
    // Split-K state (only used for normal GEMM).
    ...
    CUTLASS_DEVICE bool get_next_block(uint32_t& m_block_idx, uint32_t& n_block_idx) {
        ...
        if constexpr (kGemmType == GemmType::MGroupedMasked) {
            ...
            if constexpr (kSplitKFactor > 1) {
                DG_STATIC_ASSERT(kGemmType == GemmType::Normal, "Split-K only supports normal GEMM");
                // Keep K-splits outside MN so adjacent CTAs remain in the same
                // split and 2-CTA clusters preserve their multicast pairing.
